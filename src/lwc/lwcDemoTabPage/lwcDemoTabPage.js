...
import getLeadRecord from '@salesforce/apex/LwcDemoController.getLeadList';
import deleteLeadRecord from '@salesforce/apex/LwcDemoController.deleteLead';
...
export default class LwcDemoTabPage extends LwcComBase {
    columns = COLS;
    searchKey = '';
    inputKey = '';
    selectedRows = [];
            ...
            this.gfn_Confirm('삭제 확인','선택된 문서를 삭제하시겠습니까?','headerless')
                .then(result => {
                    if(result){
                        this.gfn_ShowSpinner();
                        console.log(row.Id);
                        deleteLeadRecord({recordId: row.Id})
                            .then(data=>{
                                this.selectedRows = [];
